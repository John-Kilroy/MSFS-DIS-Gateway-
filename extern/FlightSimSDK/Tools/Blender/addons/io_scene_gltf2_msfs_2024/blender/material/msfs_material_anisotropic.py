...
class MSFS2024_Anisotropic(MSFS2024_Material):
    ...
    def customShaderTree(self):
        super(MSFS2024_Anisotropic, self).defaultShadersTree()
    ...
    def anisotropicShaderTree(self): # TODO - Update shader tree for anisotropic -> Lucas ?
        anisotropicFrame = addNode(
            nodes = self.nodes,
            name = MSFS2024_FrameNodes.anisotropicFrame.name(),
            typeNode = MSFS2024_ShaderNodeTypes.nodeFrame.value,
            color = (0.35, 0.6, 0.1)
        )
        ## Anisotropic Texture
        # Out[0] : Separate Anisotrpic -> In[0]
        anisotropicTexNode = addNode(
            nodes = self.nodes,
        ...
        separateAnisotropicNode = addNode(
            nodes = self.nodes,
            ...
            frame = anisotropicFrame)
        # Links
        link(self.links, anisotropicTexNode.outputs[0], separateAnisotropicNode.inputs[0])

    def setAnisotropicTex(self, tex):
        nodeAnisotropicTex = getNodeByName(self.nodes, MSFS2024_AnisotropicNodes.anisotropicTex.value)
        nodeAnisotropicTex.image = tex

        nodeSeparateAnisotropic = getNodeByName(self.nodes, MSFS2024_AnisotropicNodes.separateAnisotropic.value)
        nodePrincipledBSDF = getNodeByName(self.nodes, MSFS2024_ShaderNodes.principledBSDF.value)

        if nodeAnisotropicTex.image:
            link(self.links, nodeSeparateAnisotropic.outputs[0], nodePrincipledBSDF.inputs[MSFS2024_PrincipledBSDFInputs.anisotropic.value])
            link(self.links, nodeSeparateAnisotropic.outputs[2], nodePrincipledBSDF.inputs[MSFS2024_PrincipledBSDFInputs.anisotropicRotation.value])
        else:
            unLinkNodeInput(self.links, nodePrincipledBSDF, MSFS2024_PrincipledBSDFInputs.anisotropic.value)
            unLinkNodeInput(self.links, nodePrincipledBSDF, MSFS2024_PrincipledBSDFInputs.anisotropicRotation.value)

    
    @staticmethod
    def drawPanel(layout, material):
        MSFS2024_Anisotropic.drawParametersPanel(layout, material)
        MSFS2024_Anisotropic.drawTexturesPanel(layout, material)
