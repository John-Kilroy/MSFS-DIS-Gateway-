...
class AsoboSkyPortalLight:
    bl_options = {"UNDO"}

    ExtensionName = "ASOBO_sky_portal"
    ...
    def create(vnode, gltf_node):
        ...
        extension = gltf_node.extensions.get(AsoboSkyPortalLight.ExtensionName)
        if not extension:
            return

        bpy.ops.msfs2024.add_light(name = gltf_node.name, msfs_light_type="skyPortalLight")
        ...
        for extensionParameter in AsoboSkyPortalLight.ExtensionParameters:
        ...
        if MSFS2024LightPropertiesEnum.lightShape.extensionName() and extension.get(MSFS2024LightPropertiesEnum.lightShape.extensionName()) is not None:
            extensionShapeTypeIndex = extension.get(MSFS2024LightPropertiesEnum.lightShape.extensionName())
            for shapeType, index in AsoboSkyPortalLight.ShapeTypeMatchTable.items():
                if index == extensionShapeTypeIndex:
                    setattr(
                        properties, 
                        MSFS2024LightPropertiesEnum.lightShape.attributeName(), 
                        shapeType
                    )
    ...
    def removeLightObject(vnode, gltf2_node, blender_node):
        ...
        if gltf2_node.extensions:
            extension = gltf2_node.extensions.get(AsoboSkyPortalLight.ExtensionName)
            if extension:
    ...
    def export(gltf2_object, blender_object):
        ...
        if getattr(blender_light, MSFS2024LightPropertiesEnum.lightType.attributeName()) != "skyPortalLight":
            return

        msfs_light_properties = blender_light.msfs_light_properties
        if MSFS2024LightPropertiesEnum.lightShape.extensionName():
            shapeTypeIndex = AsoboSkyPortalLight.ShapeTypeMatchTable[getattr(msfs_light_properties, MSFS2024LightPropertiesEnum.lightShape.attributeName())]
            extension[MSFS2024LightPropertiesEnum.lightShape.extensionName()] = shapeTypeIndex
        ...
        gltf2_object.extensions[AsoboSkyPortalLight.ExtensionName] = Extension(
            name=AsoboSkyPortalLight.ExtensionName,
            extension=extension,
            required=False
        )