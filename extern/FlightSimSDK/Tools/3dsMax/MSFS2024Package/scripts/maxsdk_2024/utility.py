...
from maxsdk_2024.globals import MAXVERSION, MAX2017, RT

if MAXVERSION() < MAX2017:
    import MaxPlus


def roundToUpper(number, decimals = 0):
    offset = math.pow(10, decimals)
...
def isDeprecatedExportPath(exportPath):
    return exportPath.lower().endswith(".gltf")

def getFixedDeprecatedExportPath(exportPath):
    return os.path.dirname(exportPath) + "\\"

def fixDeprecatedExportPath(exportPath):
    if isDeprecatedExportPath(exportPath):
        return getFixedDeprecatedExportPath(exportPath)
    return exportPath
