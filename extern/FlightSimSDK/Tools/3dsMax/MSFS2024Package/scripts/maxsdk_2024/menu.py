...
def deleteCategoryAndContentCustomizations(categoryName):
    ...
    MSFS2024Menu = rt.menuman.findmenu(categoryName)
    if( MSFS2024Menu is not None):
        items = getAllItem( MSFS2024Menu)
        for item in items:
            MSFS2024Menu.removeItem(item)
...
def getMenu(menuName):
    menu = rt.menuMan.findMenu(menuName)
    if not menu:
        menu = rt.menuMan.createMenu(menuName)
    return menu
