...
namespace MSFS2024_Max2Babylon
{
    public partial class BabylonExporter
    {
        ...
        readonly Dictionary<IIGameSkin,IIGameNode> skinNodeMap = new Dictionary<IIGameSkin, IIGameNode>();
        ...
        {
            List<IIGameNode> bones = new List<IIGameNode>();
        ...
        private List<IIGameNode> GetSubModelSkinHierarchy(IIGameSkin skin, IIGameNode meshNode)
        {
            ...
            if (bones.Count == 0)
            {
                logger?.RaiseWarning($"[BABYLON][WARNING][Skin] Skin of {meshNode.Name} has no bones.", logRank);
                return new List<IIGameNode>();
            }

            if (bones.Contains(null))
            {
                logger?.RaiseError($"[BABYLON][ERROR][Skin] Skin of {meshNode.Name} has bones that are outside of the exported hierarchy.", logRank);
                logger?.RaiseError($"[BABYLON][ERROR][Skin] Skin of {meshNode.Name}  cannot be exported", logRank);
                return new List<IIGameNode>();
            }
            ...
            {
                logger?.RaiseError($"[BABYLON][ERROR][Skin] More than one root node for the Skin of {meshNode.Name}. The skeleton bones need to be part of the same hierarchy.", logRank);
                logger?.RaiseError($"[BABYLON][ERROR][Skin] The Skin of {meshNode.Name} cannot be exported.", logRank);
        ...
        /// </returns>
        private readonly Dictionary<IIGameSkin, List<IIGameNode>> relevantNodesBySkin = new Dictionary<IIGameSkin, List<IIGameNode>>();
        ...
        private IIGameNode GetLowestCommonAncestor(IIGameNode nodeA, IIGameNode nodeB, List<IIGameNode> nodeHierarchyA = null, List<IIGameNode> nodeHierarchyB = null)
        {
            ...
            IIGameNode lowestCommonAncestor;
            while (true)
            {
                --hierarchyAIndex;
                --hierarchyBIndex;
        ...
        /// </returns>
        private readonly Dictionary<IIGameSkin, List<int>> nodeIndexBySkin = new Dictionary<IIGameSkin, List<int>>();
