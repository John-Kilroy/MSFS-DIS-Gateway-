...
namespace MSFS2024_Max2Babylon
{
	public partial class BabylonExporter
	{
		...
		private void ExportMaterial(IIGameMaterial materialNode, BabylonScene babylonScene, bool exportSubMat = true)
		{
		   ...
		   logger?.RaiseMessage(name, 1);
			...
			#region prints
			{
				...
				{
#if MAX2024
					logger?.RaiseVerbose("Texture[" + i + "] is named '" + materialNode.MaxMaterial.GetSubTexmapSlotName(i, true) + "'", 2);
#else
			...
			if (exportSubMat &&  materialNode.SubMaterialCount > 0)
			{
				...
				for (var index = 0; index < materialNode.SubMaterialCount; index++)
				{
					...
					if (subMat != null)
					{
						...
						if (!referencedMaterials.Contains(subMat))
						{
							...
							ExportMaterial(subMat, babylonScene, (subMat.SubMaterialCount == 0));
						}
					}
					else
					{
			...
			else if (stdMat != null)
			{
				...
				{
					logger?.RaiseWarning($"[BABYLON][WARNING][Material][Texture] Diffuse texture named {babylonMaterial.diffuseTexture.originalPath} is a .tif file and its Alpha Source is 'Image Alpha' by default.", 2);
					logger?.RaiseWarning($"[BABYLON][WARNING][Material][Texture] If you don't want material to be in BLEND mode, set diffuse texture Alpha Source to 'None (Opaque)'", 2);
				}
				...
				if (isUnlit == false)
				{
					...
					if (fresnelParameters != null)
					{
						...
						{
							logger?.RaiseWarning("[BABYLON][WARNING][Material][Texture] Fallout cannot be used with reflection channel without a texture", 2);
						}
						else
						{
					...
					babylonMaterial.diffuseTexture.hasAlpha && !babylonMaterial.opacityTexture.getAlphaFromRGB)
				{
					...
					logger?.RaiseWarning("[BABYLON][WARNING][Material][Texture] Opacity texture was removed because alpha from diffuse texture can be use instead", 2);
					logger?.RaiseWarning("[BABYLON][WARNING][Material][Texture] If you do not want this behavior, just set Alpha Source = None on your diffuse texture", 2);
				}
				...
				if (babylonAttributesContainer != null)
				{
					...
					if (babylonMaterial.isUnlit)
					{
						...
						{
							logger?.RaiseWarning("[BABYLON][WARNING][Material] Material is unlit. Emission is discarded and replaced by diffuse.", 2);
						}
			...
			else if (isPhysicalMaterial(materialNode))
			{
				...
				ITexmap colorTexmap = GetTexMap(materialNode, 1);
				ITexmap alphaTexmap = null;
				if (isTransparencyModeFromBabylonAttributes == false || babylonMaterial.transparencyMode != 0)
				{
					alphaTexmap = GetTexMap(materialNode, 9);
				}
				...
				if (isUnlit == false)
				{
					...
					ITexmap metallicTexmap = GetTexMap(materialNode, 5);
					ITexmap roughnessTexmap = GetTexMap(materialNode, 4);
					ITexmap ambientOcclusionTexmap = GetTexMap(materialNode, 6); // Use diffuse roughness map as ambient occlusion
						...
						if (metallicTexmap != null || roughnessTexmap != null)
						{
							...
							if (ambientOcclusionTexmap != null)
							{
								if (exportParameters.mergeAOwithMR)
								{
									...
									// coordinate indices
									var ambientOcclusionTexture = GetBitmapTex(ambientOcclusionTexmap);
									...
									if (texCoordIndex != ormTexture.coordinatesIndex)
									{
										...
										var uvGen = ExportUV(ambientOcclusionTexture.UVGen, babylonMaterial.occlusionTexture);
									}
									else
									{
										babylonMaterial.occlusionTexture = ormTexture;
			...
			else if (isArnoldMaterial(materialNode))
			{
				...
				if (coatWeight > 0.0f)
				{
					...
					ITexmap intensityTexmap = GetTexMap(materialNode, 23);
					ITexmap roughnessTexmap = GetTexMap(materialNode, 25);
				...
				if (isTransparencyModeFromBabylonAttributes == false || babylonMaterial.transparencyMode != 0)
				{
					alphaTexmap = GetTexMap(materialNode, "opacity");
				}
				...
				{
					// Metallic, roughness
					ITexmap metallicTexmap = GetTexMap(materialNode, 9);
					ITexmap roughnessTexmap = GetTexMap(materialNode, 5);
					ITexmap ambientOcclusionTexmap = GetTexMap(materialNode, 6); // Use diffuse roughness map as ambient occlusion
					...
					{
#if MAX2024
						if (materialNode.MaxMaterial.GetSubTexmapSlotName(i, true) == "normal")
						{
							babylonMaterial.normalTexture = ExportPBRTexture(materialNode, i, babylonScene);
						}

						else if (materialNode.MaxMaterial.GetSubTexmapSlotName(i, true) == "emission")
						{
			...
			else
			{
				...
				throw new System.Exception($"[BABYLON][ERROR][Material] Material : {materialNode.MaterialName} has unsupported Material type: {materialNode.MaterialClass}");
		...
		/// <returns></returns>
		public IIGameMaterial IsMaterialSupported(IIGameMaterial materialNode,BabylonScene babylonScene, bool isSubMat = false)
		{
			// MSFS2024 Material
			if (MSFS2024_Max2Babylon.FlightSim.FlightSimMaterialUtilities.IsMSFS2024Material(materialNode.MaxMaterial))
			{
				return null;
			}
			...
			if (materialNode.SubMaterialCount > 0 && !isSubMat)
			{
				...
				for (int indexSubMaterial = 0; indexSubMaterial < materialNode.SubMaterialCount; indexSubMaterial++)
				{
					...
					IIGameMaterial unsupportedSubMaterial = IsMaterialSupported(subMaterialNode, babylonScene, true);
		...
		private IIGameMaterial GetRenderMaterialFromDirectXShader(IIGameMaterial materialNode)
		{
			...
			if (isDirectXShaderMaterial(materialNode))
			{
				...
				{
					logger?.RaiseWarning($"[BABYLON][WARNING][Material] DirectX material property for {materialNode.MaterialName} is null...", 2);
				}
