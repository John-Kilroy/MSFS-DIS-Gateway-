...
namespace MSFS2024_Max2Babylon
{
    public partial class BabylonExporter
    {
        ...
        private BabylonCamera ExportCamera(IIGameScene gameScene, IIGameNode cameraNode, BabylonScene babylonScene)
        {
            ...
            IIGameCamera gameCamera = cameraNode.IGameObject.AsGameCamera();
            ICameraObject maxCamera = gameCamera.MaxObject as ICameraObject;
            bool initialized = gameCamera.InitializeData;
            BabylonCamera babylonCamera = new BabylonCamera
            {
                name = cameraNode.Name,
                id = cameraNode.MaxNode.GetGuid().ToString(),
                metadata = ExportExtraAttributes(cameraNode, babylonScene),
                fov = Tools.ConvertFovToVertical(maxCamera.GetFOV(0, Tools.Forever))
            };
            ...
            // Target
            IIGameNode target = gameCamera.CameraTarget;
            if (target != null)
            {
            ...
            else
            {
                ...
                List<BabylonAnimation> extraAnimations = new List<BabylonAnimation>();
