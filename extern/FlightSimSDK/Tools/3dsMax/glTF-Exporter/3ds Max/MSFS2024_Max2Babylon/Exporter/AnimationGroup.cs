...
namespace MSFS2024_Max2Babylon
{
	...
	public class AnimationGroupList : List<AnimationGroup>
	{
		...
		public void LoadFromData(IINode dataNode = null)
		{
			...
			nodePropDictionary.TryGetValue(s_AnimationListPropertyName, out string animProp);
		...
		public static AnimationGroupList InitAnimationGroups(ILoggingProvider logger)
		{
			...
			if (animationList.Count > 0)
			{
				...
				foreach (AnimationGroup animGroup in animationList)
				{
					...
					if (warnings.Count > 0)
					{
						logger?.RaiseWarning("[BABYLON][WARNING][Animation] Animation Group -> " + animGroup.Name, 2);
		...
		private static void ResolveMultipleInheritedContainer(IIContainerObject container)
		{
			...
			// resolve nodes , adding an id at the end
			{
				...
				foreach (IINode n in containerHierarchy)
				{
					...
					if (n.Mtl != null && FlightSimMaterialUtilities.HasMSFS2024Materials(n.Mtl) && FlightSimMaterialUtilities.HasRuntimeAccess(n.Mtl))
					{
						if (n.Mtl.IsMultiMtl)
						{
			...
			container.ContainerNode.SetUserPropBool("flightsim_resolved", false);
		}
