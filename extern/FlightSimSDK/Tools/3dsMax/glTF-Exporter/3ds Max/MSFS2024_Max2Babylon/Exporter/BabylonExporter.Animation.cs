...
namespace MSFS2024_Max2Babylon
{
	public partial class BabylonExporter
	{
		private IList<BabylonAnimationGroup> ExportAnimationGroups(BabylonScene babylonScene)
		{
			...
			logger?.Print("[BABYLON][Animations] Init Animation Groups", Color.Black);
			...
			foreach (AnimationGroup animGroup in animationList)
			{
				...
				Dictionary<string, BabylonNode> nodes = new Dictionary<string, BabylonNode>();
				Dictionary<string, BabylonBone> bones = new Dictionary<string, BabylonBone>();
				...
				foreach (Guid guid in animGroup.NodeGuids)
				{
					...
					// node could have been deleted or not selected we ignore it
					if (maxNode == null)
						continue;
					...
					// Node
					babylonScene.NodeMap.TryGetValue(nodeId, out BabylonNode node);

					if (node != null)
					{
						if (exportParameters.exportAsSubmodel && node.extraNode)
						{
							continue;
						}

						if (node.animations != null && !nodes.ContainsKey(nodeId))
						{
							...
							{
								RemoveStaticAnimations(ref animations, animationGroup);
							}
					...
					if (bone != null)
					{
						if (bone.animation != null && !bones.ContainsKey(boneId))
						{
				...
				if (animationGroup.targetedAnimations.Count > 0)
				{
					if (nodes.Count() > 0)
					{
						logger?.RaiseWarning("[BABYLON][Animations] Animation Group : " + animGroup.Name);
						foreach (var node in nodes.Values)
						{
							logger?.RaiseWarning("\t -> Exported Animation Node(s) | " + node.name);
						}
					}
		...
		private static bool ExportBabylonKeys(List<BabylonAnimationKey> keys, string property, List<BabylonAnimation> animations, BabylonAnimation.DataType dataType, BabylonAnimation.LoopBehavior loopBehavior)
		{
			...
			int end = Loader.Core.AnimRange.End;
			...
			BabylonAnimation babylonAnimation = new BabylonAnimation
			{
		...
		{
			List<BabylonAnimationKey> keys = ExportBabylonKeysFromGameController(control, type, extractValueFunc);
		...
		{
			IILinFloatKey key = Loader.Global.ILinFloatKey.Create();
		...
		private static bool ExportController(IControl control, string property, List<BabylonAnimation> animations, uint classId, BabylonAnimation.DataType dataType, Func<int, IIKeyControl, BabylonAnimationKey> generateFunc)
		{
			...
			if (!(control.GetInterface(InterfaceID.Keycontrol) is IIKeyControl keyControl))
			{
				return false;
			}
		...
		private static bool ExportQuaternionController(IControl control, string property, List<BabylonAnimation> animations)
		{
			...
			return ExportController(control, property, animations, 0x2003, BabylonAnimation.DataType.Quaternion,
				(index, keyControl) =>
				{
					IILinRotKey key = Loader.Global.ILinRotKey.Create();
					keyControl.GetKey(index, key);
					IQuat newQuat = key.Val;
		...
		private static bool ExportVector3Controller(IControl control, string property, List<BabylonAnimation> animations)
		{
			...
			if (ExportController(control, property, animations, 0x2002, BabylonAnimation.DataType.Vector3,
				(index, keyControl) =>
				{
					IILinPoint3Key key = Loader.Global.ILinPoint3Key.Create();
					keyControl.GetKey(index, key);
			...
			return ExportController(control, property, animations, 0x2004, BabylonAnimation.DataType.Vector3,
				(index, keyControl) =>
				{
					IILinScaleKey key = Loader.Global.ILinScaleKey.Create();
					keyControl.GetKey(index, key);
		...
		{
			ExportAnimation(property, animations, extractValueFunc, BabylonAnimation.DataType.Color3, true);
		}
		...
		private void ExportVector3Animation(string property, List<BabylonAnimation> animations,
			Func<int, float[]> extractValueFunc)
		{
			ExportAnimation(property, animations, extractValueFunc, BabylonAnimation.DataType.Vector3,true);
		}

		private void ExportQuaternionAnimation(string property, List<BabylonAnimation> animations,
			Func<int, float[]> extractValueFunc)
		{
			ExportAnimation(property, animations, extractValueFunc, BabylonAnimation.DataType.Quaternion, true);
		}

		public void ExportFloatAnimation(string property, List<BabylonAnimation> animations,
			Func<int, float[]> extractValueFunc)
		{
			ExportAnimation(property, animations, extractValueFunc, BabylonAnimation.DataType.Float, true);
		}
		...
		private bool IsAnEmptyAnimation(BabylonAnimation babylonAnimation)
		{
			...
			BabylonAnimationKey defaultValue = babylonAnimation.keysFull[0];
		...
		{
			OptimizeAnimations(keys, removeLinearAnimationKeys, out _);
		}
		...
		{
			BabylonAnimationKey first = keys[ixFirst];
			BabylonAnimationKey middle = keys[ixFirst + 1];
			BabylonAnimationKey last = keys[ixFirst + 2];
			...
			{
				float[] computedMiddleValue = WeightedLerp((int)first.frame, (int)middle.frame, (int)last.frame, first.values, last.values);
		...
		public void RemoveStaticAnimations(ref IList<BabylonAnimation> babylonAnimations, BabylonAnimationGroup babylonAnimationGroup)
		{
		...
		{
			int start = Loader.Core.AnimRange.Start;
			int end = Loader.Core.AnimRange.End;
			List<BabylonAnimationKey> keys = new List<BabylonAnimationKey>();

			for (int key = start; key <= end; key += Loader.Global.TicksPerFrame)
			{
		...
		public void GeneratePositionAnimation(IIGameNode gameNode, List<BabylonAnimation> animations)
		{
				...
				gameNode.IGameControl.IsAnimated(IGameControlType.PosZ))
			{
				...
				{
					IGMatrix localMatrix = gameNode.GetLocalTM(key);

					if (float.IsNaN(localMatrix.Determinant))
					{
						logger?.RaiseError($"[BABYLON][ERROR][Animations] Determinant of {gameNode.Name} of position animation at {key} localMatrix is NaN ");
					}

					BabylonMatrix tm_babylon = new BabylonMatrix
					{
						m = localMatrix.ToArray()
					};
		...
		public void GenerateRotationAnimation(IIGameNode gameNode, List<BabylonAnimation> animations, bool force = false)
		{
				...
				force)
			{
				ExportQuaternionAnimation("rotationQuaternion", animations, key =>
				{
					...
					if (float.IsNaN(localMatrix.Determinant))
					{
						logger?.RaiseError($"[BABYLON][ERROR][Animations] Determinant of {gameNode.Name} of rotation animation at {key} localMatrix is NaN ");
					}
		...
		public void GenerateScalingAnimation(IIGameNode gameNode, List<BabylonAnimation> animations)
		{
			if (gameNode.IGameControl.IsAnimated(IGameControlType.Scale))
			{
				ExportVector3Animation("scaling", animations, key =>
				{
					...
					if (float.IsNaN(localMatrix.Determinant))
					{
						logger?.RaiseError($"[BABYLON][ERROR][Animations] Determinant of {gameNode.Name} of scale animation at {key} localMatrix is NaN ");
					}
