	...
	public class GLTFChannelCameraTarget : GLTFProperty
	{
	 ...
	 public class FlightSimCameraPropertyAnimationExporter : IBabylonExtensionExporter
	 {
		...
		public object ExportGLTFExtension<T1,T2>(T1 babylonObject, ref T2 gltfObject, ref GLTF gltf, GLTFExporter exporter, ExtensionInfo info)
		{
			...
			if (!(babylonObject is BabylonCamera babylonCamera)) return null;

			Guid.TryParse(babylonCamera.id, out Guid guid);
			...
			if (maxCamera== null)
			{
				string message = $"[GLTFExporter][ERROR][Camera] Impossible to export {FlightSimAsoboPropertyAnimationExtension.SerializedName} for {camNode.Name}, camera node must be a TargetCamera type";
				throw new System.Exception(message);
			}

			if (babylonCamera != null && gltfObject is GLTFAnimation gltfAnimation && FlightSimCameraUtilities.IsMSFSCamera(camNode))
			{
				GLTFCamera gltfCamera = null;
				if (!exporter.nodeToGltfNodeMap.TryGetValue(babylonCamera, out GLTFNode gltfNode)) return gltfObject;
				...
				if (samplerIndex >-1)
				{
					...
					GLTFAnimationPropertyChannel propertyChannel = new GLTFAnimationPropertyChannel
					{
						...
						target = $"cameras/{gltfCamera.index}/perspective/yfov"
					};
		...
		public bool ExportBabylonExtension<T>(T babylonObject, ref BabylonScene babylonScene, BabylonExporter exporter)
		{
			...
			if (isGLTFExported && FlightSimCameraUtilities.IsMSFSCamera(cameraNode.CameraTarget.MaxNode))
			{
				_ = cameraNode.CameraTarget.MaxNode.GetGuid().ToString();
		...
		private BabylonCamera ExportCameraAnimations(BabylonCamera babylonCamera, IIGameCamera camera, BabylonExporter exporter)
		{
			...
			for (int i = 0; i < numProps; ++i)
			{
				...
				_ = property.MaxParamBlock2?.GetParamDef(property.ParamID);
		...
		private int AddParameterSamplerAnimation(BabylonCamera babylonCamera, GLTFAnimation gltfAnimation, GLTFExporter exporter, GLTF gltf, int startFrame, int endFrame)
		{
			...
			if (babylonAnimations.Count > 0)
			{
				...
				foreach (BabylonAnimation babylonAnimation in babylonAnimations)
				{
					...
					{
						exporter.logger.RaiseWarning(String.Format("[GLTFExporter][WARNING][Animation] No frames to export in material animation \"{1}\" of node named \"{0}\". This will cause an error in the output gltf.", babylonCamera.name, babylonAnimation.name));
					}
