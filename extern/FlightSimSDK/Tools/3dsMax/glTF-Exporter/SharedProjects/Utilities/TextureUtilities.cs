...
namespace Utilities
{
	static class TextureUtilities
    {
        ...
        public static string TryWriteImage(string outputFolder, string sourcePath, string textureName, ILoggingProvider logger, ExportParameters exportParameters)
        {
            ...
            {
                logger?.RaiseWarning("[GLTFExporter][WARNING][Material][Texture] Texture path is missing.", 3);
                return null;
            }
            ...
            if (validImageFormat == null)
            {
                ...
                logger?.RaiseWarning(string.Format("[GLTFExporter][WARNING][Material][Texture] Format of texture {0} is not supported by the exporter. Consider using a standard image format like jpg or png.", Path.GetFileName(sourcePath)), 3);
                return null;
            }
            ...
            if (File.Exists(absolutePath))
            {
                try
                {
                    switch (Path.GetExtension(absolutePath).ToLower())
                    {
                        ...
                        default:
                            logger?.RaiseError(string.Format("[ERROR][Texture] Format of texture {0} is not supported by the exporter. Consider using a standard image format like jpg or png.", Path.GetFileName(absolutePath)), 3);
                            return null;
                    }
                }
                catch (Exception e)
                {
                    logger?.RaiseError(string.Format("[ERROR][Texture] Failed to load texture {0}: {1}", Path.GetFileName(absolutePath), e.Message), 3);
                    return null;
                }
            }
            else
            {
                logger?.RaiseError(string.Format("[ERROR][Texture] Texture {0} not found.", absolutePath), 3);
                return null;
            }
        }
        ...
        private static void _copyTexture(string sourcePath, string destPath, long imageQuality, List<string> validFormats, List<string> invalidFormats, ILoggingProvider logger)
        {
            try
            {
                if (File.Exists(sourcePath))
                {
                    ...
                    {
                        logger?.RaiseError(string.Format("[ERROR][Texture] Format of texture {0} is not supported by the exporter. Consider using a standard image format like jpg or png.", Path.GetFileName(sourcePath)), 3);
                    }
                }
                else logger?.RaiseError(string.Format("[ERROR][Texture] Texture not found: {0}", sourcePath), 3);
            }
            catch (Exception c)
            {
                logger?.RaiseError(string.Format("[ERROR][Texture] Exporting Texture {0} failed: {1}", sourcePath, c.ToString()), 3);
            }
        }
        ...
        private static void _convertToBitmapAndSave(string sourcePath, string destPath, string imageFormat, long imageQuality, ILoggingProvider logger)
        {
            ...
            switch (imageFormat)
            {
                case "dds":
                    ...
                    {
                        logger?.RaiseError(string.Format("[ERROR][Texture] Failed to convert texture {0} to png: {1}", Path.GetFileName(sourcePath), e.Message), 3);
                    }
                    break;
                case "tga":
                    // External library TargaImage.dll
        ...
        public static void SaveBitmap(Bitmap bitmap, string directoryName, string fileName, ImageFormat imageFormat, long imageQuality, ILoggingProvider logger)
        {
            ...
            {
                logger?.RaiseError($"[ERROR][Texture] Failed to save bitmap: directory name '{directoryName}' contains invalid character{(invalidCharsInString.Count > 1 ? "s" : "")} {invalidCharsInString.ToArray().ToString(false)}", 3);
                return;
            }
            invalidCharsInString = GetInvalidChars(fileName, Path.GetInvalidFileNameChars());
            if (invalidCharsInString.Count > 0)
            {
                logger?.RaiseError($"[ERROR][Texture] Failed to save bitmap: file name '{fileName}' contains invalid character{(invalidCharsInString.Count > 1 ? "s" : "")} {invalidCharsInString.ToArray().ToString(false)}", 3);
                return;
            }
