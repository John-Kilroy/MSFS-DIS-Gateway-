    ...
    partial class GLTFExporter
    {
        private void ExportMaterial(BabylonMaterial babylonMaterial, GLTF gltf)
        {
            ...
            if (gltfMaterial.index != -1 && babylonMaterial.isUnlit)
            {
                ...
                {
                    logger?.RaiseWarning("[GLTFExporter][WARNING][Materials] KHR_materials_unlit has not been enabled for export!", 2);
                }
                else
                {
            ...
            materialToGltfMaterialMap.Add(babylonMaterial,gltfMaterial);
        }
        ...
        private void getAlphaMode(BabylonStandardMaterial babylonMaterial, out string alphaMode, out float? alphaCutoff)
        {
            ...
            switch (babylonMaterial.transparencyMode)
            {
                ...
                case (int)BabylonPBRMetallicRoughnessMaterial.TransparencyMode.ALPHATESTANDBLEND:
                    logger?.RaiseWarning("[GLTFExporter][WARNING][Materials] Alpha test and blend mode is not supported in glTF. Alpha blend is used instead.", 3);
                    alphaMode = GLTFMaterial.AlphaMode.BLEND.ToString();
                    break;
                default:
                    logger?.RaiseWarning("[GLTFExporter][WARNING][Materials] Unsupported transparency mode: " + babylonMaterial.transparencyMode, 3);
                    break;
            }
        }
