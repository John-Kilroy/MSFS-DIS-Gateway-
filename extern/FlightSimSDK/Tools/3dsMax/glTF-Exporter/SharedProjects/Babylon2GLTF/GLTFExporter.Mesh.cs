...
namespace Babylon2GLTF
{
    partial class GLTFExporter
    {
        ...
        private readonly Dictionary<GLTFMesh, List<GLTFMesh>> sharedSkinnedMeshesByOriginal = new Dictionary<GLTFMesh, List<GLTFMesh>>();
        ...
        private GLTFMesh ExportMesh(BabylonMesh babylonMesh, GLTF gltf, BabylonScene babylonScene)
        {
            ...
            {
                // logger?.RaiseWarning($"[GLTFExporter][Mesh] Mesh {babylonMesh.name} is a dummy. It will be ignored.");
                return null;
            }
            ...
            logger?.RaiseMessage("[GLTFExporter][Meshs] Init glTF", 2);
            
            // Mesh
            var gltfMesh = new GLTFMesh { name = babylonMesh.name };
            if (!gltf.MeshesList.Any(x => x.name == babylonMesh.name))
            {
                logger?.Print($"[GLTFExporter][Mesh] Exporting Mesh : {babylonMesh.name}", Color.Black);
                ...
                if (babylonMorphTargetManager != null)
                {
                    ...
                    {
                        morphTargetNames.Add(babylonMorphTarget.name);
                        weights.Add(babylonMorphTarget.influence);
                    }
                    gltfMesh.weights = weights.ToArray();
                    object[] extraMorphTargetsNames = morphTargetNames.Cast<object>().ToArray();

                    // We need to save the target names in the extras data of the mesh
                    gltfMesh.extras = new Dictionary<string, object>
                    {
                        { FlightSimExtensionUtility.Name_ASOBO_morphTargetNames, extraMorphTargetsNames }
                    };
                }
        ...
        private BabylonMorphTargetManager GetBabylonMorphTargetManager(BabylonScene babylonScene, BabylonMesh babylonMesh)
        {
            if (babylonMesh.morphTargetManagerId.HasValue)
            {
                ...
                {
                    logger?.RaiseWarning("[GLTFExporter][WARNING][Mesh] Morph Targets Managers is not defined", 3);
                }
                else
                {
                    ...
                    if (babylonMorphTargetManager == null)
                    {
                        logger?.RaiseWarning($"[GLTFExporter][WARNING][Mesh] Morph Targets Managers with index {babylonMesh.morphTargetManagerId.Value} not found", 3);
                    }
        ...
        private void _exportMorphTargets(BabylonMesh babylonMesh, BabylonSubMesh babylonSubMesh, BabylonMorphTargetManager babylonMorphTargetManager, GLTF gltf, GLTFBuffer buffer, GLTFMeshPrimitive meshPrimitive)
        {
            ...
            foreach (var babylonMorphTarget in babylonMorphTargetManager.targets)
            {
                ...
                {
                    GLTFAccessor accessorTargetPositions = GLTFBufferService.Instance.CreateAccessor(
                        gltf,
                        GLTFBufferService.Instance.GetBufferViewFloatVec3(gltf, buffer),
                ...
                {
                    GLTFAccessor accessorTargetNormals = GLTFBufferService.Instance.CreateAccessor(
                        gltf,
                        GLTFBufferService.Instance.GetBufferViewFloatVec3(gltf, buffer),
                ...
                // TextCoord
                if (babylonMorphTarget.uvs != null)
                {
                    var accessorUVs = GLTFBufferService.Instance.CreateAccessor(
                        gltf,
                        GLTFBufferService.Instance.GetBufferViewFloatVec2(gltf, buffer),
                    ...
                    gltfMorphTarget.Add(GLTFMorphTarget.Attribute.TEXCOORD_0.ToString(), accessorUVs.index);

                    // Populate accessor
                    int nbComponents = 2; // Vector2
                    int startIndex = babylonSubMesh.verticesStart * nbComponents;
                    int endIndex = startIndex + babylonSubMesh.verticesCount * nbComponents;
                    for (int indexUV = startIndex; indexUV < endIndex; indexUV += 2)
                    {
                        var uvTarget = ArrayExtension.SubArray(babylonMorphTarget.uvs, indexUV, 2);
                        var uvMesh = ArrayExtension.SubArray(babylonMesh.uvs, indexUV, 2);
                        for (int indexCoordinate = 0; indexCoordinate < uvTarget.Length; indexCoordinate++)
                        {
                            uvTarget[indexCoordinate] = uvTarget[indexCoordinate] - uvMesh[indexCoordinate];
                        }

                        // Store values as bytes
                        foreach (var coordinate in uvTarget)
                        {
                            accessorUVs.bytesList.AddRange(BitConverter.GetBytes(coordinate));
                        }
                        accessorUVs.count = babylonSubMesh.verticesCount;
                    }
                }
                // --- Colors ---
                if (babylonMorphTarget.colors != null)
                {
                    GLTFAccessor accessorColors = GLTFBufferService.Instance.CreateAccessor(
                        gltf,
                        GLTFBufferService.Instance.GetBufferViewFloatVec4(gltf, buffer),
                        "accessorTargetColors",
                        GLTFAccessor.ComponentType.FLOAT,
                        GLTFAccessor.TypeEnum.VEC4
                    );
                    gltfMorphTarget.Add(GLTFMeshPrimitive.Attribute.COLOR_0.ToString(), accessorColors.index);

                    // Populate accessor
                    int nbComponents = 4; // Vector4
                    int startIndex = babylonSubMesh.verticesStart * nbComponents;
                    int endIndex = startIndex + babylonSubMesh.verticesCount * nbComponents;
                    for (int indexColors = startIndex; indexColors < endIndex; indexColors += 4)
                    {
                        var colorsTarget = ArrayExtension.SubArray(babylonMorphTarget.colors, indexColors, 4);
                        var colorsMesh = ArrayExtension.SubArray(babylonMesh.colors, indexColors, 4);
                        for (int indexCoordinate = 0; indexCoordinate < colorsTarget.Length; indexCoordinate++)
                        {
                            colorsTarget[indexCoordinate] = colorsTarget[indexCoordinate] - colorsMesh[indexCoordinate];
                        }

                        // Store values as bytes
                        foreach (var coordinate in colorsTarget)
                        {
                            accessorColors.bytesList.AddRange(BitConverter.GetBytes(coordinate));
                        }
                        accessorColors.count = babylonSubMesh.verticesCount;
                    }
                }
