...
namespace Babylon2GLTF
{
	partial class GLTFExporter
	{
		...
		private readonly Dictionary<BabylonSkeleton, BabylonSkeletonExportData> alreadyExportedSkeletons = new Dictionary<BabylonSkeleton, BabylonSkeletonExportData>();
		...
		private GLTFSkin ExportSkin(BabylonSkeleton babylonSkeleton, GLTF gltf, GLTFNode gltfNode, GLTFMesh gltfMesh)
		{
			...
			foreach (var babylonBone in babylonSkeleton.bones)
			{
				...
				{
					var nodePair = nodeToGltfNodeMap.FirstOrDefault(pair => pair.Key.id.Equals(babylonBone.id));
					if (nodePair.Equals(default(KeyValuePair<BabylonNode,GLTFNode>)) && exportParameters.exportAsSubmodel) 
					{
						throw new System.Exception($"Error retrieving submodels dependencies of the skin containing the bone : {babylonBone.name}, verify there is at least a common ancestor between the submodels and their dependencies");
					}
